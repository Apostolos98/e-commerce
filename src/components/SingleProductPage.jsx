...
export default function ProductPage({ product, hideDropDown, setHideDropDown, hideCart, setHideCart, loading }) {
    const { addToCart } = useContext(CartContext)
    const [index, setInd] = useState(null)
    const [primarySrc, setSrc] = useState(null)
    const [item, setItem] = useState(null)
    const [starStyle, setStarStyle] = useState(null)
    let url;
    ...
    useEffect(() => {
        if (product) {
            ...
            url = product.images.filter((el) => el.rel === 'Front_Standard')[0]
            url = url===undefined?product.images[0]:url
            setItem({name: product.name, url: url, price: product.salePrice})
